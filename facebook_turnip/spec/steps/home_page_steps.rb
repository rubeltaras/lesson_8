step 'there is a facebook home page' do
  Capybara.reset_session!
  @home_page = HomePage.new
  @home_page.load
end

step 'user open home page' do
   @home_page = HomePage.new
   @home_page.load
end

step 'user fill form with existing credentials' do
  @home_page.login.set "userfortestpurpose@gmail.com"
  @home_page.password_main.set "somepassword123"
  @home_page.login.click
end

step 'press login button' do
  @home_page.login_button.click
end

step 'user should be logged in' do
  expect(@home_page.find_friends.text).to eq("Find Friends")
end

step 'user fill form with :used_email' do |email|
...
step 'click on sign_up button' do
  @home_page.sign_up.click
end

step 'user should see error message that email is already registered' do
  expect(@home_page.existing_user_alert.text).to eq("There is an existing account associated with this email.")
end

step 'user should see error message that he is not able to continue registation' do
  expect(@home_page.sorry_message.text).to eq("Sorry, we are not able to process your registration.")
end

step 'fill form with blank :exception'  do |exception|
  data = {
      "first_name" => "user",
...
step 'user should see error message and stay on home page' do
